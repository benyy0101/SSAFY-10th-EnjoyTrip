...
const curDate = ref(startDate.value.format("YYYY.MM.DD"));
...
watch(
  ...
  () => {
    initSavedOption();
    curDate.value = startDate.value
        .add(props.curPage - 1, "day")
        .format("YYYY.MM.DD");
  },
  { deep: true }
);
...
<template>
  ...
  <div v-else>
    <a-popover placement="topRight">
      ...
      <a-float-button
        class="next-button"
        @click="$emit('savePlan', chosenOption)"
      >
        <template #icon>
          <CheckOutlined />
        </template>
